...
function*/
void pushtoken(square board[NUM_ROWS][NUM_COLUMNS], player players[],int j,int selectedSquare)//function prototype
{
...
void pushtokenabove(square board[NUM_ROWS][NUM_COLUMNS], player players[],int j,int selectedSquare)//function prototype
{
...
from the function*/
int counttokens(square board[NUM_ROWS][NUM_COLUMNS],int selectedSquare)/*function prototype*/
{
  ...
  token is present*/
int gettokenCol(square board[NUM_ROWS][NUM_COLUMNS],int random)/*function prototype*/
{
  int moveCol=-2;
  int a=0;
 ...
 for(i=0;i<eachtoken;i++)
  {
      ...
      { /*asks the user to select a square*/
        ...
        scanf("%d",&squareselected);//inputs the squareselected
        ...
        {
          pushtoken(board,players,j,squareselected);//push token function
        }
        ...
        else if(counttokens(board,squareselected)==minNumOfTokens && board[squareselected][0].stack_top->col!=players[j].col)
             {/*calls the pushtokenabove function */
               pushtokenabove(board,players,j,squareselected);
             }
        else
         {/*if the above conditions are not true then 
          prints on screen to enter again*/
            printf("\n Wrong square selected.Please enter again");
            scanf("%d",&squareselected);//accepts again
         }
  ...
  void pushnext(square board[NUM_ROWS][NUM_COLUMNS],int sRow,int sCol)
  {
  ...
  void sidestepmove(square board[NUM_ROWS][NUM_COLUMNS],token *s,int sRow,int sCol,int dir)
  {
    ...
    switch(dir)
     {
      ...
      case 1: s->next=board[sRow+1][sCol].stack_top;
              board[sRow+1][sCol].stack_top=s;
              break;
      default:printf("No sidestpping taken\n");
    }
  }
  void tokenassignment(square board[NUM_ROWS][NUM_COLUMNS],int sRow,int sCol,token *e)
  {
    ...
    for(i=0;i<numPlayers;i++)
    {
     ...
     /*the dice is rolled and a random number is generated*/
      ...
      if(w==1)
      {
       ...
       pushnext(board,sRow,sCol);
       sidestepmove(board,st,sRow,sCol,dir);
       int moveCol=-1;
       moveCol=gettokenCol(board,rand);
     /*if the square is normal then the following 
     operations are done*/
      if(board[rand][moveCol].type==NORMAL)
      {
        token *o = board[rand][moveCol].stack_top;
        pushnext(board,rand,moveCol);
        o->next=NULL;
        moveCol==moveCol+1;
        tokenassignment(board,rand,moveCol,o);
        }
        ...
        in the other rows*/
         else if(board[rand][moveCol].type==OBSTACLE)
         {
          ...
          if(colFull==true)
          {
           ...
           tokenassignment(board,rand,moveCol,t);
          }
    ...
    for(r=0;r<6;r++)
      {
       ...
       {
        players[i].numTokensLastCol= players[i].numTokensLastCol+1;
       }
      }
